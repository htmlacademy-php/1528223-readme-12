...
function datetime_relative($datetime) {
	...
	if ($num < 60) {
		$datetime = $num . ' ' . get_noun_plural_form ($num, 'минута', 'минуты', 'минут');
		return $datetime;
	}
	
	$num = ceil($num / 60);			// число часов с округлением в большую сторону
	if ($num < 24) {
		$datetime = $num . ' ' . get_noun_plural_form ($num, 'час', 'часа', 'часов');
		return $datetime;
	}
	
	$num = ceil($num / 24);			// число дней с округлением в большую сторону
	if ($num < 7) {
		$datetime = $num . ' ' . get_noun_plural_form ($num, 'день', 'дня', 'дней');
		return $datetime;
	}
	
	$num = ceil($num / 7);			// число недель с округлением в большую сторону
	if ($num < 5) {
		$datetime = $num . ' ' . get_noun_plural_form ($num, 'неделя', 'недели', 'недель');
		return $datetime;
	}
	...
	if ($num < 12) {
		$datetime = $num . ' ' . get_noun_plural_form ($num, 'месяц', 'месяца', 'месяцев');
		return $datetime;
	} else {
