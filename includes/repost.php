...
if (isset($_GET['repost']) !== false) {
    // если $_GET['repost'] есть, то очищаем
    $repost_id = filter_input(INPUT_GET, 'repost', FILTER_SANITIZE_NUMBER_INT) ?? false;
    ...
    if (count($repost) > 0) {
        // если это репоста репоста, то в original_author
        // вставляем самого первого автора поста, а если нет, то автора поста
        $original_author = ($repost['original_author'] == null ? $repost['user_id'] : $repost['original_author']);

        // затем вставляем их в ту же таблицу, но с другими
        // id автора, временем, признаком репоста и автором оригинального поста
        $sql = '
            INSERT INTO posts
            SET
