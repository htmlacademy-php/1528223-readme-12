...
// если нажата кнопка "Подписаться", то подписываем пользователя $user_id
// на пользователя $get_id, внося соответствующую запись в БД
if (isset($_POST['subscribe']) !== false) {
    $sql = '
        INSERT IGNORE INTO subscribes SET
    ...
    // id текущего пользователя (подписчика)
    $subscriber_id = (isset($_SESSION['user'])) ? filter_var($_SESSION['user'], FILTER_SANITIZE_SPECIAL_CHARS) : false;   $_SESSION['user'];
    // имя текущего пользователя (подписчика)
    $subscriber_name = (isset($_SESSION['username'])) ? filter_var($_SESSION['username'], FILTER_SANITIZE_SPECIAL_CHARS) : false;   $_SESSION['user'];
    ...
    */
    ...
    $message->setBody('Здравствуйте, ' . $sourcer_name
    . '. На вас подписался новый пользователь '
    . $subscriber_name . '. Вот ссылка на его профиль: <a href="profile.php?id='
    . $subscriber_id . '">ссылка на профиль</a>');
...
// если нажата кнопка "Отписаться", то отписываем пользователя $user_id
// от пользователя $get_id, удаляя соответствующую запись БД
